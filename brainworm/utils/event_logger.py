    ...
    def __init__(self, project_root: Path, hook_name: str, enable_event_logging: bool = False, session_id: str = None):
        ...
        try:
            ...
            if current_state.get("correlation_id") is None:
                state_mgr._update_unified_state({"correlation_id": self.correlation_id})
        except Exception:
            ...
            if self.event_store:
                try:
                    ...
                    if debug and success:
                        ...
                        print(
                            f"📊 Event logged (v{schema_version}) [DB]: {self.session_id[:8]} in {self.hook_name}",
                            file=sys.stderr,
                        )
    ...
    def log_post_tool_execution(self, input_data: dict, debug: bool = False) -> bool:
        ...
        try:
            if timing_file.exists():
                ...
                if debug:
                    print(
                        f"✅ Timing coordination successful: {timing_info['execution_duration_ms']:.2f}ms",
                        file=sys.stderr,
                    )
            else:
...
def create_event_logger(
    project_root: Path, hook_name: str, enable_event_logging: bool = False, session_id: str = None
) -> SessionEventLogger:
