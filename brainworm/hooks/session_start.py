...
# Reads plugin location from unified state

STATE_FILE=".brainworm/state/unified_session_state.json"

if [ ! -f "$STATE_FILE" ]; then
    echo "Error: Brainworm not initialized"
    echo "Start a Claude Code session to initialize brainworm in this project"
    exit 1
fi
...
if [ -z "$PLUGIN_ROOT" ]; then
    echo "Error: Plugin path not found in state"
    echo "Start a new Claude Code session to refresh plugin location"
    exit 1
fi

if [ ! -d "$PLUGIN_ROOT" ]; then
    echo "Error: Plugin not found at $PLUGIN_ROOT"
    echo "Plugin may have been uninstalled. Start a Claude Code session to refresh."
    exit 1
fi

# Execute command from plugin location
...
'''
        ...
        # Plugin launcher wrapper (for slash commands)
        plugin_launcher_wrapper = f'''#!/usr/bin/env bash
# Generated by brainworm session_start hook
# Generic plugin script launcher for slash commands
...
exec uv run "${{PLUGIN_ROOT}}/hooks/$1" "${{@:2}}"
'''
